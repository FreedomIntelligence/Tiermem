...
from ..configs.enums import MemoryType
from ..configs.prompts import (
    PROCEDURAL_MEMORY_SYSTEM_PROMPT,
    get_update_memory_messages,
)
from ..exceptions import ValidationError as Mem0ValidationError
from .base import MemoryBase
from .setup import mem0_dir, setup_config
...
class Memory(MemoryBase):
    def __init__(self, config: MemoryConfig = MemoryConfig()):
        ...
        print(f"config: {self.config.llm.config}")
        ...
        if hasattr(self.embedding_model, "embed"):
            _orig_embed = self.embedding_model.embed

            def _embed_wrapped(text, memory_action=None):
                t0 = time.time()
                out = _orig_embed(text, memory_action)
            ...
            self.embedding_model.embed = _embed_wrapped  # type: ignore[assignment]

        # wrap llm
        if hasattr(self.llm, "generate_response"):
            _orig_gen = self.llm.generate_response

            def _gen_wrapped(*args, **kwargs):
                t0 = time.time()
                out = _orig_gen(*args, **kwargs)
                latency_ms = int((time.time() - t0) * 1000)
            ...
            self.llm.generate_response = _gen_wrapped  # type: ignore[assignment]
        ...
        try:
            ...
            if not response.strip():
                ...
                try:
                    ...
                    new_retrieved_facts = json_repair.loads(extracted_json)["facts"]
        except Exception as e:
            logger.error(f"Error in new_retrieved_facts: {e}")
            new_retrieved_facts = []

        if not new_retrieved_facts:
            logger.debug("No new facts retrieved from input. Skipping memory creation.")
            return []

        returned_memories = []
        for fact in new_retrieved_facts:
            try:
                fact_embeddings = self.embedding_model.embed(fact, "add")
                ...
                logger.error(f"Error creating memory for inferred fact: {e}")
...
class AsyncMemory(MemoryBase):
    ...
    async def _add_to_vector_store(
        ...
        # 每个 fact 独立：各自检索 old memories + 各自调用一次 LLM 生成 actions（可并发）
        async def build_fact_context(new_mem_content: str) -> Dict[str, Any]:
            ...
            retrieved_old_memory = [{"id": mem.id, "text": mem.payload.get("data", "")} for mem in existing_mems]
            ...
            function_calling_prompt = get_update_memory_messages(
                retrieved_old_memory, [new_mem_content], self.config.custom_update_memory_prompt
            )
            return {
                "new_mem": new_mem_content,
                "function_calling_prompt": function_calling_prompt,
                "temp_uuid_mapping": temp_uuid_mapping,
            }

        fact_contexts: list = []
        if new_retrieved_facts:
            fact_contexts = await asyncio.gather(*[build_fact_context(fact) for fact in new_retrieved_facts])

        async def call_update_llm(prompt: str) -> Dict[str, Any]:
            """
            调用 LLM 生成 memory actions，带简单重试机制：
            - 默认重试 3 次，可通过环境变量 MEM0_LLM_MAX_RETRIES / MEM0_LLM_RETRY_DELAY 控制
            - 适用于 5xx / 网络波动等暂时性错误（统一通过异常捕获实现）
            """
            try:
                max_retries = int(os.getenv("MEM0_LLM_MAX_RETRIES", "3") or "3")
            except Exception:
                max_retries = 3
            try:
                base_delay = float(os.getenv("MEM0_LLM_RETRY_DELAY", "1.0") or "1.0")
            except Exception:
                base_delay = 1.0
            ...
            while attempt < max_retries:
                ...
                try:
                    ...
                    logger.error(
                        f"Error in new memory actions response (attempt "
            ...
            try:
                if not resp or not str(resp).strip():
                    ...
                    return {}
                resp_str = remove_code_blocks(str(resp))
        ...
        actions_by_fact = []
        if fact_contexts:
            actions_list = await asyncio.gather(*[call_update_llm(fc["function_calling_prompt"]) for fc in fact_contexts])
            actions_by_fact = list(zip(fact_contexts, actions_list))

        returned_memories = []
        try:
            memory_tasks = []
            for fc, new_memories_with_actions in actions_by_fact:
                temp_uuid_mapping = fc.get("temp_uuid_mapping") or {}
                for resp in (new_memories_with_actions.get("memory", []) if isinstance(new_memories_with_actions, dict) else []):
                    logger.info(resp)
                    try:
                        ...
                        if event_type == "ADD":
                            ...
                            mapped_id = temp_uuid_mapping.get(resp.get("id"))
                            if not mapped_id:
                                logger.warning(f"Skipping UPDATE due to unmapped id (async): {resp.get('id')}")
                                continue
                            task = asyncio.create_task(
                                self._update_memory(
                                    memory_id=mapped_id,
                            ...
                            memory_tasks.append((task, resp, "UPDATE", mapped_id))
                        elif event_type == "DELETE":
                            mapped_id = temp_uuid_mapping.get(resp.get("id"))
                            if not mapped_id:
                                logger.warning(f"Skipping DELETE due to unmapped id (async): {resp.get('id')}")
                                continue
                            task = asyncio.create_task(self._delete_memory(memory_id=mapped_id))
                            memory_tasks.append((task, resp, "DELETE", mapped_id))
